...
Gli algoritmi di questo homework operano su grafi non pesati,
 non diretti e connessi. I grafi letti in input sono considerati grafici semplici, ma essi possono diventare multigrafi a seguito dell'invocazione del metodo \codeinline{contract}, definito nel paragrafo "Contrazione di due nodi" della sezione \ref{par:contraction}. \\
...
\begin{obser}
Rispetto alla versione dell'algoritmo vista in classe, invece di eliminare i due vertici estremi all'arco contratto e creare un terzo vertice che rappresenti i primi due, abbiamo optato per eliminare un solo vertice e trasferirne i lati rimantenti al secondo vertice. Il risultato finale e gli altri step dell'algoritom di Karger sono equivalenti, ma il codice dell'implementazione risulta essere più snello e più chiaro.
\end{obser}
...
\noindent Grazie a \mintinline{c++}{std::shared_ptr} è possibile passare il grafo originale come argomento ad altre sottoprocedure mantenendo l'ownership del puntatore alla procedura chiamante. L'oggetto viene quindi deallocato della memoria Heap solo il \codeinline{main} del programma termina. \mintinline{c++}{std::unique_ptr} è invece usato per disfarsi del grafo contratto ritornato da \codeinline{full\_contraction} al termine di ogni iterazione dell'algoritmo di Karger.
